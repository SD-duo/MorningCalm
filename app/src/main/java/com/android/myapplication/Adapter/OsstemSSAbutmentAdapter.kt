...
class OsstemSSAbutmentAdapter(
    private val onItemClick: (Mc2.ResultData.Result) -> Unit
) :
    RecyclerView.Adapter<OsstemSSAbutmentAdapter.ViewHolder>() {
    ...
    fun setItems(items: List<Mc2.ResultData.Result>) {
        ...
        this.items.clear()
        this.items.addAll(items.filter { it.category == "6" })
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): OsstemSSAbutmentAdapter.ViewHolder {
    ...
    override fun onBindViewHolder(holder: OsstemSSAbutmentAdapter.ViewHolder, position: Int) {
    ...
    inner class ViewHolder(val binding: FragmentFixtureItemBinding) :
        ...
        fun bindItems(item: Mc2.ResultData.Result) = binding.apply {
            ...
            ivImg.load(R.drawable.ssabt)
            tvName.text = item?.name
            tvSize.text =
                item?.diameter.toString() + " ø " + " x " + item?.height.toString() + " mm " + item?.gheight.toString() + " G/H"
